...
.tab-btn { padding:8px 16px; border-radius:20px; font-size:12px; font-weight:600; border:none; cursor:pointer; transition:all .2s; }
.tab-btn.active { background:var(--dingley); color:#fff; }
.tab-btn:not(.active) { background:transparent; color:var(--dingley); }
</style>
</head>
<body>
...
<div class="max-w-3xl mx-auto px-4 py-5">
  ...
  {{-- TABS --}}
  <div style="background:#fff;border-radius:12px;border:1px solid #e8e4dc;padding:6px;display:flex;gap:4px;margin-bottom:16px;">
  ...
  <div class="space-y-3" id="orderList">
    ...
    <div class="card p-4 order-item" data-status="{{ $status }}">
      ...
      <div class="flex items-start justify-between mb-3">
        ...
        <span style="background:{{ $statusColor['bg'] }};color:{{ $statusColor['text'] }};font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;">
          {{ $statusLabel }}
        </span>
      </div>
      ...
      <div style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--cream);border-radius:10px;margin-bottom:10px;">
        <div style="width:48px;height:48px;border-radius:8px;overflow:hidden;background:#e8e4dc;flex-shrink:0;">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p style="font-size:11px;color:var(--dingley);margin:0;">
            Qty: {{ $order->quantity ?? 1 }} &bull; {{ $order->product->store->name ?? '-' }}
          </p>
        </div>
      ...
      {{-- Pickup Code --}}
      @if($order->pickup_code && in_array($status, ['confirmed','picked_up','siap diambil']))
      <div style="background:var(--kombu);border-radius:10px;padding:10px 14px;display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
        <div>
          ...
          <p style="color:var(--brandy);font-size:20px;font-weight:700;letter-spacing:4px;margin:0;">{{ $order->pickup_code }}</p>
        </div>
        <svg class="w-8 h-8" style="color:var(--dingley)" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
      </div>
      @endif
      ...
      <div style="display:flex;align-items:center;gap:0;font-size:10px;">
        @php
          ...
          $stepOrder = ['pending','confirmed','picked_up'];
          $currentIdx = array_search($status, $stepOrder) ?? 0;
        @endphp
        @foreach($steps as $key => $label)
        @php $idx = array_search($key, $stepOrder); $done = $idx <= $currentIdx; @endphp
        <div style="display:flex;align-items:center;flex:1;">
          <div style="display:flex;flex-direction:column;align-items:center;gap:2px;">
            <div style="width:20px;height:20px;border-radius:50%;background:{{ $done ? 'var(--dingley)' : '#e8e4dc' }};display:flex;align-items:center;justify-content:center;">
              @if($done)
              <svg class="w-3 h-3" fill="none" stroke="#fff" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
              @endif
            </div>
            <p style="color:{{ $done ? 'var(--dingley)' : '#aaa' }};font-weight:{{ $done ? '600' : '400' }};margin:0;white-space:nowrap;">{{ $label }}</p>
          </div>
          @if(!$loop->last)
