...
<div class="flex items-center justify-between mb-5">
    <div>
        <p class="text-xs text-gray-400 mt-0.5">Total {{ $categories->count() }} kategori terdaftar</p>
    </div>
...
<div class="grid grid-cols-3 gap-4 mb-5">
    ...
    @php
        $colors = [
            ...
            'red'    => ['bg' => 'bg-red-50',    'text' => 'text-red-700',    'border' => 'border-red-100',    'badge' => 'bg-red-100 text-red-700'],
            'purple' => ['bg' => 'bg-purple-50', 'text' => 'text-purple-700', 'border' => 'border-purple-100', 'badge' => 'bg-purple-100 text-purple-700'],
            'pink'   => ['bg' => 'bg-pink-50',   'text' => 'text-pink-700',   'border' => 'border-pink-100',   'badge' => 'bg-pink-100 text-pink-700'],
        ];
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 hover:border-gray-200 transition-all">
        <div class="flex items-start justify-between mb-3">
            <div class="flex items-center gap-2.5">
                <div class="w-10 h-10 {{ $c['bg'] }} {{ $c['border'] }} border rounded-xl flex items-center justify-center text-lg">
                    {{ $cat->icon }}
                </div>
                <div>
        ...
        <p class="text-xs text-gray-400 mb-4 min-h-[32px]">{{ $cat->description ?? 'Tidak ada deskripsi' }}</p>
        ...
        <div class="flex items-center gap-1.5 pt-3 border-t border-gray-50">
            ...
            <button onclick="openEdit({{ $cat->id }}, '{{ addslashes($cat->name) }}', '{{ addslashes($cat->description) }}', '{{ $cat->icon }}', '{{ $cat->color }}')"
                class="flex-1 text-xs text-center py-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50">
                Edit
            </button>
            ...
            <form method="POST" action="{{ route('admin.categories.toggle', $cat->id) }}" class="flex-1">
                @csrf
                <button class="w-full text-xs text-center py-1.5 rounded-lg border
...
{{-- Modal Tambah --}}
<div id="modalTambah" class="hidden fixed inset-0 z-50 flex items-center justify-center" style="background:rgba(0,0,0,0.3)">
    <div class="bg-white rounded-xl border border-gray-100 p-6 w-[440px]">
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-2">
                        ...
                        <label class="cursor-pointer">
                            ...
                            <span class="w-9 h-9 flex items-center justify-center text-lg rounded-lg border border-gray-100 peer-checked:border-green-400 peer-checked:bg-green-50 hover:bg-gray-50">
                                {{ $emoji }}
                            </span>
                        </label>
                        @endforeach
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <label class="cursor-pointer">
                            ...
                            <span class="w-7 h-7 rounded-full {{ $cls }} flex items-center justify-center peer-checked:ring-2 peer-checked:ring-offset-2 peer-checked:ring-{{ $val }}-500">
                            </span>
                        </label>
                        @endforeach
                    </div>
            ...
            <div class="flex gap-2 mt-5">
                <button type="button" onclick="document.getElementById('modalTambah').classList.add('hidden')"
                    class="flex-1 text-xs py-2 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50">
                    Batal
                </button>
                <button type="submit"
                    class="flex-1 text-xs py-2 rounded-lg bg-green-600 text-white hover:bg-green-700">
                    Simpan Kategori
                </button>
            </div>
        </form>
...
{{-- Modal Edit --}}
<div id="modalEdit" class="hidden fixed inset-0 z-50 flex items-center justify-center" style="background:rgba(0,0,0,0.3)">
    <div class="bg-white rounded-xl border border-gray-100 p-6 w-[440px]">
        <div class="flex items-center justify-between mb-4">
        ...
        <form method="POST" id="formEdit">
            ...
            <div class="space-y-3">
                ...
                <div>
                    <label class="text-xs text-gray-500 mb-2 block">Icon Emoji <span class="text-red-400">*</span></label>
                    <div class="flex flex-wrap gap-2 mb-2" id="editIconGrid">
                        @foreach(['🍱','🥗','🍰','🥐','🍜','🍔','🥩','🍣','🥤','🍫','🥙','🍛'] as $emoji)
                        <label class="cursor-pointer">
...
function openEdit(id, name, desc, icon, color) {
    ...
    document.querySelectorAll('.edit-icon').forEach(r => {
        r.checked = r.value === icon;
    });
    document.querySelectorAll('.edit-color').forEach(r => {
